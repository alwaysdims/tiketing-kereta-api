@extends('admin_kai.layout.main', ['title' => 'Users Petugas'])
...
<div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow overflow-x-auto">
        <table class="w-full text-sm text-gray-700" id="tablePetugas">
            ...
            <tbody id="tableBody">
                @forelse($petugas as $i => $user)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="px-4 py-3">{{ $petugas->firstItem() + $i }}</td>
                        <td class="px-4 py-3">{{ $user->petugas->nama_petugas ?? '-' }}</td>
                        <td class="px-4 py-3">{{ $user->petugas->nip ?? '-' }}</td>
                        <td class="px-4 py-3">{{ $user->username }}</td>
                        <td class="px-4 py-3">{{ $user->email }}</td>
                        <td class="px-4 py-3">{{ $user->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</td>
                        <td class="px-4 py-3">{{ $user->petugas->jabatan ?? '-' }}</td>
                        <td class="px-4 py-3">{{ $user->petugas->shift ?? '-' }}</td>
                        <td class="px-4 py-3">{{ $user->petugas->no_hp ?? '-' }}</td>
                        <td class="px-4 py-3">{{ $user->petugas->alamat ?? '-' }}</td>
                        <td class="px-4 py-3">
                            <div class="flex space-x-2">
                                ...
                                <form action="{{ route('admin.users.petugas.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Yakin hapus?')">
                                    @csrf
                                    @method('DELETE')
...
<div id="addModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 py-10">
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <form method="POST" action="{{ route('admin.users.petugas.store') }}">
                @csrf
                <div class="mb-3">
                    <label class="block text-sm font-medium text-gray-700">Nama Petugas *</label>
                    <input type="text" name="nama_petugas" value="{{ old('nama_petugas') }}" required class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('nama_petugas') border-red-500 @enderror">
                    @error('nama_petugas')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nip" value="{{ old('nip') }}" required class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('nip') border-red-500 @enderror">
                    @error('nip')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <select name="jabatan" class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('jabatan') border-red-500 @enderror">
                        <option value="">Pilih</option>
                        <option value="Loket" {{ old('jabatan') == 'Loket' ? 'selected' : '' }}>Loket</option>
                        <option value="Boarding" {{ old('jabatan') == 'Boarding' ? 'selected' : '' }}>Boarding</option>
                        <option value="Kondektur" {{ old('jabatan') == 'Kondektur' ? 'selected' : '' }}>Kondektur</option>
                        <option value="Masinis" {{ old('jabatan') == 'Masinis' ? 'selected' : '' }}>Masinis</option>
                    </select>
                    @error('jabatan')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-10">
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <form id="editForm" method="POST">
                ...
                <div class="mb-3">
                    <label class="block text-sm font-medium text-gray-700">Nama Petugas *</label>
                    <input type="text" name="nama_petugas" id="editNama" required class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('nama_petugas') border-red-500 @enderror">
                    @error('nama_petugas')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    <label class="block text-sm font-medium text-gray-700">Jabatan</label>
                    <select name="jabatan" id="editJabatan" class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('jabatan') border-red-500 @enderror">
                        <option value="">Pilih</option>
                        <option value="Loket">Loket</option>
                        <option value="Boarding">Boarding</option>
                        <option value="Kondektur">Kondektur</option>
...
<div id="detailModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm md:max-w-md my-10 transform transition-all duration-300 scale-95 hover:scale-100 ease-out">
        <div class="p-6">
            <h2 class="text-2xl font-bold text-gray-800 mb-5">Detail Petugas</h2>
...
<script>
    ...
    const petugas = @json($petugas->items());
    let currentPage = 1;
    let pageSize = 5;
    let filteredData = [...petugas];
    ...
    function applyFilters() {
        ...
        filteredData = petugas.filter(user => {
            const matchesSearch = (
                (user.petugas?.nama_petugas?.toLowerCase() || '').includes(searchTerm) ||
                (user.petugas?.nip?.toLowerCase() || '').includes(searchTerm) ||
                (user.username?.toLowerCase() || '').includes(searchTerm) ||
                (user.email?.toLowerCase() || '').includes(searchTerm) ||
                (user.petugas?.jabatan?.toLowerCase() || '').includes(searchTerm) ||
                (user.petugas?.shift?.toLowerCase() || '').includes(searchTerm) ||
                (user.petugas?.no_hp?.toLowerCase() || '').includes(searchTerm) ||
                (user.petugas?.alamat?.toLowerCase() || '').includes(searchTerm)
            );
            const matchesShift = selectedShift ? user.petugas?.shift === selectedShift : true;
            const matchesJenisKelamin = selectedJenisKelamin ? user.jenis_kelamin === selectedJenisKelamin : true;
            const matchesJabatan = selectedJabatan ? user.petugas?.jabatan === selectedJabatan : true;
            ...
            row.innerHTML = `
                ...
                <td class="px-4 py-3">
                    <div class="flex space-x-2">
                        ...
                        <form action="/admin/users/petugas/${user.id}" method="POST" onsubmit="return confirm('Yakin hapus?')">
    ...
    function showEdit(user) {
        ...
        document.getElementById('editNama').value = user.petugas?.nama_petugas ?? '';
        document.getElementById('editNip').value = user.petugas?.nip ?? '';
        document.getElementById('editJabatan').value = user.petugas?.jabatan ?? '';
        document.getElementById('editShift').value = user.petugas?.shift ?? '';
        document.getElementById('editNoHp').value = user.petugas?.no_hp ?? '';
        document.getElementById('editAlamat').value = user.petugas?.alamat ?? '';
        ...
        document.getElementById('editForm').action = `/admin/users/petugas/${user.id}`;
        showModal('editModal');
    }

    function showDetail(user) {
        document.getElementById('detailNama').innerText = user.petugas?.nama_petugas ?? '-';
        document.getElementById('detailNip').innerText = user.petugas?.nip ?? '-';
        ...
        document.getElementById('detailJabatan').innerText = user.petugas?.jabatan ?? '-';
        document.getElementById('detailShift').innerText = user.petugas?.shift ?? '-';
        document.getElementById('detailNoHp').innerText = user.petugas?.no_hp ?? '-';
        document.getElementById('detailAlamat').innerText = user.petugas?.alamat ?? '-';
    ...
    @if ($errors->any())
        ...
        @elseif (session('showEditModal'))
            showModal('editModal');
            @if (session('editPetugas'))
                showEdit(@json(session('editPetugas')));
            @endif
        @endif
    @endif
</script>
