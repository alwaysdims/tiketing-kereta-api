...
<div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="flex justify-between mb-6">
        <h1 class="text-2xl font-bold">Data Penumpang</h1>
        ...
        <table class="w-full text-sm">
            ...
            <tbody>
                @forelse($penumpangs as $i => $user)
                    <tr class="border-b">
                        <td class="px-4 py-2">{{ $penumpangs->firstItem() + $i }}</td>
                        <td class="px-4 py-2">{{ $user->penumpang->nama_penumpang ?? '-' }}</td>
                        <td class="px-4 py-2">{{ $user->nik }}</td>
                        <td class="px-4 py-2">{{ $user->username }}</td>
                        <td class="px-4 py-2">{{ $user->email }}</td>
                        <td class="px-4 py-2">{{ $user->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</td>
                        <td class="px-4 py-2">{{ $user->penumpang->no_hp ?? '-' }}</td>
                        <td class="px-4 py-2">{{ $user->penumpang->alamat ?? '-' }}</td>
                        <td class="px-4 py-2">
                            <div class="flex space-x-2">
                                ...
                                <button type="button" onclick='showEdit(@json($user))' class="text-green-500 hover:text-green-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                                         stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                              d="M11 4H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2v-5M18.5 2.5a2.121
                                              2.121 0 113 3L12 15l-4 1 1-4 9.5-9.5z"/>
                                    </svg>
                                </button>

                                <!-- Hapus -->
                                <form action="{{ route('admin.users.penumpang.destroy', $user->id) }}" method="POST"
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <form method="POST" action="{{ route('admin.users.penumpang.store') }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nama_penumpang" class="w-full border rounded px-2 py-1">
                </div>
                <div class="mb-3">
                    <label>Tanggal Lahir</label>
                    <input type="date" name="tanggal_lahir" class="w-full border rounded px-2 py-1">
                </div>
                <div class="mb-3">
                    <label>No HP</label>
    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-10">
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <form id="editForm" method="POST">
                ...
                <div class="mb-3">
                    <label>Nama Penumpang</label>
                    <input type="text" name="nama_penumpang" id="editNama" class="w-full border rounded px-2 py-1">
                </div>
                <div class="mb-3">
                    <label>Tanggal Lahir</label>
                    <input type="date" name="tanggal_lahir" id="editTanggalLahir" class="w-full border rounded px-2 py-1">
                </div>
                <div class="mb-3">
                    <label>No HP</label>
